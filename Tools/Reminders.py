...
def set_new_reminder(tool_call: Tools.ToolCall.ToolCall) -> str:
  ...
  return "Let Izzy know you've set the reminder, and give her a confirmation :)"
...
def get_reminders(tool_call: Tools.ToolCall.ToolCall) -> str:
  ...
  if len(results) == 0:
    return "Let Isabelle know there are no reminders currently set"
  ...
  if count < 4:
    ...
    final_result = json.dumps({
      "result": "Tell Izzy a summary of the reminders. If she asks for more detail then you can say more!",
      "reminders": results
    })
...
def get_reminders_semantically(tool_call: Tools.ToolCall.ToolCall) -> str:
  ...
  if count == 1:
    final_result = json.dumps({
      "result": "Ask Isabelle if this is the reminder she meant.",
      "reminder": results[0]
    })
...
def remove_reminder(tool_call: Tools.ToolCall.ToolCall) -> str:
  ...
  try:
    ...
    if reminder[0].score < 7:
      confirm = json.dumps({
        "Instruction": f"Ask Isabelle if she meant to delete: {reminder[0].reminder.abstract}."
      })
    ...
    return "Let Izzy know that the backend failed to remove the reminder."
    
  return "Let Izzy know the reminder has been removed!"
